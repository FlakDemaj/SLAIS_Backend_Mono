...
| `Commands/UpdateNightShiftTemplate` | `{ TemplateGuid, ExpectedVersion, Key, TensionStart, SortOrder, German?, English? }` | `GetNightShiftTemplateResponseDto` | `TemplateNotFound`; `ExpectedVersion != Version` → `TemplateModifiedByOtherUser`; Key-Änderung nur im Zustand `Pending`, sonst `TemplateStateTransitionInvalid` (-520015, Handler); Key belegt durch andere Vorlage → `TemplateKeyAlreadyExists`; mitgelieferte Texte ersetzen (`AddOrReplaceText`), fehlende bleiben |
