...
public class GetNightShiftSessionQueryHandler : BaseHandler<GetNightShiftSessionQuery>,
    IRequestHandler<GetNightShiftSessionQuery, NightShiftSessionDetailResponseDto>
{
    private readonly INightShiftSessionRepository _nightShiftSessionRepository;

    public GetNightShiftSessionQueryHandler(
        INightShiftSessionRepository nightShiftSessionRepository,
        IMapper mapper,
        ISlaisLogger<GetNightShiftSessionQuery> logger)
        : base(mapper, logger)
    {
        _nightShiftSessionRepository = nightShiftSessionRepository;
    }

    public async Task<NightShiftSessionDetailResponseDto> HandleAsync(
        GetNightShiftSessionQuery request,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return new NightShiftSessionDetailResponseDto
        {
            ...
            Messages = _mapper.Map<List<NightShiftSessionMessageDto>>(session.Messages
                ...
                .ToList()),
            Feedback = session.Feedback == null ? null : ToFeedback(session.Feedback, session.Language)
        };
    }

    private NightShiftFeedbackResponseDto ToFeedback(
        Domain.Simulations.NightShift.NightShiftFeedbackEntity feedback,
        Language language)
    {
